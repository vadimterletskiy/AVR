...
//  Compiler....: любой
//
//  Description.: ћакросы дл€ работы с битами, пинами и виртуальными портами 
//
//  Data........: 24.05.2013 —оздан
//
//***************************************************************************
...
ќпредел€ем виртуальный или реальный порт - псевдоним  название, буква, тип порта
------------------------------------
псевдоним - любой
название - любое
буква - дл€ виртуальных портов люба€
        дл€ реальных A, B, C или D ...
тип порта - _VIRT, _REAL, _NONE
_NONE - исключает порт из кода. дл€ порта с таким атрибутом определены 
пустые макросы
_VIRT может использоватьс€ с реальным портом, но в этом случае будет генерироватьс€
неоптимальный код

#define LCD_PORT  LCD_DATA, F, _VIRT

ќписываем пины порта  - название  буква, вывод, акт. уровень
------------------------------------
название LCD_DATA_0 ... LCD_DATA_7
буква порта A, B, C или D ...
номер пина 0..7
активный уровень _LOW, _HI, _NONE
активный уровень используетс€ макросами включени€/выключени€ - On/Off
_NONE - исключает вывод из виртуального порта, хот€ в описании этот 
пин будет присутствовать, код дл€ него генерироватьс€ не будет. 
...
“ак можно производить запись и чтение порта
...
/*______________________макросы дл€ работы с битами _____________________________*/
...
/*___________________ макросы дл€ работы с одиночными пинами __________________*/
...
#define _PM_ClearPin(id, pin, lev)         _PM_ClearPin##lev(id, pin, lev) 
...
/*______________________ макросы дл€ работы с портами __________________________*/
...
/*______________________ установка направлени€ пинов _______________________________*/
